...
- Billing module config tables: `BillingPreference`, `BillingTaxProfile`, `WalletAutoRecharge`, `BillingPromoCode`, `BillingPromoRedemption` (migration 0051, new tables). They layer config onto the EXISTING money infra (SchoolSubscription/AIWallet/PlatformPayment/AICreditTransaction/AuditLog) — no money data duplicated. Service: services/billing.py.
- Payment methods: `BillingPaymentMethod` (migration 0052) stores only display metadata (brand/last4/expiry) + optional gateway token per school — PCI-safe, never a PAN/CVV. Service helpers in services/billing.py (list/add/update/set_default/remove, expiry-state, default promotion).
- `DocumentRegistry` (migration 0053): universal authenticity registry (uuid, document_type, payload, content_hash, status, source_type/source_id) for every generated document. Service: services/document_registry.py; public verify: routers/verify.py.
- `DocumentTemplate` (migration 0054): per-school diploma/certificate templates (background upload, {{placeholder}} title/body, fields_config, one default per school+kind). Service: services/document_templates.py; router: /document-templates.
- ReferenceItem: generic hierarchical referential table (category/code/name/sort_order/is_active; school_id NULL = GLOBAL TeducAI Super-Admin-managed, set = LOCAL to one school; unique category+code+school_id). Consumed via services/reference_data.py merged view.
- ExamSession étendu (module Scolarité → Examens, colonne-only 0056: subject_id/duration_minutes/room/max_score/coefficient/notes) — table PARTAGÉE avec la planification operations. Nouveaux modèles Vie scolaire: DisciplineRecord, SchoolActivity, HealthRecord (données médicales — accès administration), BoardingRecord (chambres = rooms de facilities).
- SEC-04/SEC-05 (migration 0057): `users.mfa_last_code` (anti-rejeu TOTP) et table `PasswordResetToken` (hash SHA-256 du jeton, expiration, usage unique, IP demandeuse).
