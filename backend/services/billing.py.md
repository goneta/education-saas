...
- Document QR: `attach_registry(db, detail, school_id, user)` registers the invoice in the document registry and injects uuid/verify_url/qr_text; `render_invoice_pdf` stamps a top-right authenticity QR (registry JSON) and prints the verify URL.
