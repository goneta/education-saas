...
- Method-first UX: `CINETPAY_NETWORK_CHANNELS` maps the user-facing mobile-money method (orange_money/mtn_money/moov_money -> MOBILE_MONEY, wave -> WALLET) to the CinetPay checkout channel; when a method is chosen the init payload narrows `channels` to that family (no method -> CINETPAY_CHANNELS default). The former `payment_method` init param (not part of the checkout v2 contract) was removed.
