...
def attach_registry(db: Session, detail: dict, school_id: int, user: Optional[models.User]) -> Optional[dict]:
    """Register the invoice in the universal document registry and inject the
    authenticity fields (uuid, verify_url, QR JSON) into the detail dict so the
    PDF embeds a verifiable QR."""
    if not detail:
        return detail
    ...
    payload = {
        "School Name": customer.get("name"),
        "Invoice": True,
        "Invoice Number": detail.get("number"),
        "Customer Name": customer.get("name"),
        "Date Generated": date_str,
        "Generated By": (user.full_name if user else None),
    }
    row = document_registry.register(
        db, document_type="invoice", school_id=school_id,
        title=f"Invoice {detail.get('number')}", reference=detail.get("number"),
        issued_to_name=customer.get("name"), payload=payload,
        source_type="invoice", source_id=detail.get("payment_id"), issued_by=user,
    )
    detail["uuid"] = row.uuid
    detail["verify_url"] = document_registry.verification_url(row.uuid)
    detail["qr_text"] = document_registry.qr_text(row)
    return detail
...
def render_invoice_pdf(detail: dict) -> bytes:
    ...
    # Authenticity QR — top-right corner (encodes the registry JSON + verify URL).
    from . import document_registry
    qr_size = 22 * mm
    qr_x = right - qr_size
    qr_y = h - 12 * mm - qr_size
    qr_text = detail.get("qr_text") or str(detail.get("verify_reference", ""))
    try:
        document_registry.draw_qr_on_canvas(c, qr_text, qr_x, qr_y, qr_size)
        c.setFont("Helvetica", 6)
        c.setFillColorRGB(0.5, 0.5, 0.5)
        c.drawCentredString(qr_x + qr_size / 2, qr_y - 3 * mm, "Scan to verify")
    except Exception:  # QR must never break invoice rendering
        pass
    rtext = qr_x - 5 * mm  # right edge for header text so it clears the QR
    ...
    c.drawRightString(rtext, y, "INVOICE")
    c.setFont("Helvetica", 10)
    c.setFillColorRGB(0.35, 0.35, 0.35)
    c.drawRightString(rtext, y - 6 * mm, f"#{detail.get('number','')}")
    date = detail.get("date")
    date_str = date.strftime("%Y-%m-%d") if hasattr(date, "strftime") else str(date or "")
    c.drawRightString(rtext, y - 11 * mm, date_str)
    ...
    c.roundRect(rtext - 30 * mm, y - 20 * mm, 30 * mm, 6 * mm, 3, fill=1, stroke=0)
    c.setFillColorRGB(1, 1, 1)
    c.setFont("Helvetica-Bold", 8)
    c.drawCentredString(rtext - 15 * mm, y - 18.2 * mm, status)
    ...
    # Payment method + verifiable reference / URL (the QR itself is top-right).
    c.setFont("Helvetica", 9)
    c.setFillColorRGB(0.3, 0.3, 0.3)
    c.drawString(left, 40 * mm, f"Payment method: {detail.get('provider','') or '-'}")
    c.drawString(left, 35 * mm, "Thank you for using TeducAI.")
    c.setFont("Helvetica", 7)
    c.setFillColorRGB(0.5, 0.5, 0.5)
    c.drawString(left, 28 * mm, f"Ref: {detail.get('verify_reference','')}")
    if detail.get("verify_url"):
        c.drawString(left, 24 * mm, f"Verify: {detail.get('verify_url')}")
...
def email_invoice(db: Session, school_id: int, payment_id: int, user: models.User,
    ...
    if not detail:
        return {}
    attach_registry(db, detail, school_id, user)
