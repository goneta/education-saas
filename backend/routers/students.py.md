...
- `GET /students/diagnostics` (admin-only, read-only): production triage for empty lists — resolved active context, stage-by-stage counts of the exact list filters (profiles total, active users with profile, school match, SMA match, enrollment match, final count), where students actually live (distinct user school_ids, profile SMA ids), the caller's preference SMA and the school's active SMAs, plus French hints naming the probable cause. Declared BEFORE /{student_id}.
- List query uses a correlated EXISTS (enrollment resolved via StudentGlobalProfile) instead of OUTER JOIN + DISTINCT. DISTINCT over the full User row 500s on PostgreSQL (`could not identify an equality operator for type json` — User.address_structured is a `json` column, no `=` operator). EXISTS produces no duplicate rows so DISTINCT is unnecessary; pagination stays correct. SQLite tests never caught it because SQLite allows DISTINCT over JSON.
- Stale-pin tolerance: a profile pinned to a model assignment that is NOT active for the school anymore (replaced/deactivated assignment) is treated like an unpinned profile in the home-school fallback (`~sma.in_(active SMA ids of the school)`), so a school's own students never silently vanish after a model change. Mirrored in the diagnostics `profile_sma_matches_context` stage. Covered by `test_students_list_tolerance.py`.
- MONEY-02: `_student_outstanding_balance` normalise le reste a payer et le blocage de certificat utilise `money.is_outstanding()` — un residu flottant bloquait la delivrance du certificat d une famille pourtant a jour.
