...
- UX-01: `POST /auth/refresh` (session glissante) — un client authentifie echange un jeton encore valide contre un jeton neuf. Ne prolonge pas une session volee (jeton revoque refuse via token_version) ni une session inactive (le minuteur d inactivite du frontend deconnecte toujours).
