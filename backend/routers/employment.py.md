...
- SEC-07: `/public-profiles` (marketplace public, opt-in triple des eleves) est desormais limite en debit et journalise par IP (`rate_limit_public_search`, access_type public_search) pour empecher le moissonnage; `/cv/{cv_id}/photo` exige la publication complete (`employment.is_publicly_listed`) au lieu du seul share_enabled, afin qu une photo partagee uniquement par sharecode ne soit plus enumerable par identifiant.
- Correctif seconde passe (REGRESSION du Lot 1): `/employment/public-profiles` renvoyait **500**. Le garde-fou SEC-07 inserait un StudentCVAccessLog pour tracer la recherche, or cette table exige `student_cv_id` NOT NULL (elle trace l acces a UN CV, pas une recherche) — la page PUBLIQUE /emploi etait cassee. La tracabilite passe desormais par le journal applicatif et le plafond anti-moissonnage par security_middleware (PUBLIC_SEARCH_PATHS).
