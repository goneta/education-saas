...
- SEC-01: `_verify_signature` delegue a `webhook_auth.verify_shared_secret` (FAIL-CLOSED). Un secret non configure en production renvoie 503 (le fournisseur reessaie, aucun effet monetaire) au lieu d accepter un appel anonyme; comparaison a temps constant; secret par fournisseur prioritaire sur SCHOOL_PAYMENT_WEBHOOK_SECRET.
