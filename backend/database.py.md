...
- Garde production CFG-01: `is_production()` (APP_ENV=production OU presence de .env.production) est desormais LA source de verite partagee (SECRET_KEY, webhooks, DB). `validate_database_url(url, production=, configured=)` (fonction pure, testee) refuse le demarrage en production si DATABASE_URL est absent ou pointe sur SQLite - avant, l app demarrait silencieusement sur un fichier local non sauvegarde.
- PERF-03: `_engine_options()` active `pool_pre_ping` (fin des 500 en rafale apres un redemarrage/idle de PostgreSQL), `pool_recycle` (DB_POOL_RECYCLE_SECONDS, 1800 par defaut) et un dimensionnement explicite (DB_POOL_SIZE / DB_MAX_OVERFLOW); SQLite conserve check_same_thread=False.
