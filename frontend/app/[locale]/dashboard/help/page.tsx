...
const HELP_SECTIONS: HelpSection[] = [
    ...
    },
    {
        ...
        icon: Wand2,
        purpose: {
            fr: "Exécuter en un clic les automatisations de l'établissement : relance des impayés avec escalade progressive, et résumé hebdomadaire aux parents (notes, absences, paiements en attente, dans la langue du parent) avec alertes de seuil. Chaque exécution est idempotente (anti-spam intégré) et peut être planifiée via un cron externe.",
            en: "Run the school's automations in one click: unpaid-fee reminders with progressive escalation, and the weekly parent digest (grades, absences, pending payments, in the parent's language) with threshold alerts. Every run is idempotent (built-in anti-spam) and can be scheduled via an external cron.",
            es: "Ejecutar con un clic las automatizaciones del centro: recordatorios de impagos con escalado progresivo y el resumen semanal para padres (notas, ausencias, pagos pendientes, en el idioma del padre) con alertas de umbral. Cada ejecución es idempotente (anti-spam integrado) y puede programarse con un cron externo.",
            sw: "Endesha otomatiki za shule kwa mbofyo mmoja: vikumbusho vya ada ambazo hazijalipwa na kupanda hatua kwa hatua, na muhtasari wa wiki kwa wazazi (alama, kutokuwepo, malipo yanayosubiri, kwa lugha ya mzazi) pamoja na tahadhari za vizingiti. Kila uendeshaji ni salama kurudiwa (kinga ya barua taka) na unaweza kupangwa kwa cron ya nje.",
        },
        steps: [
