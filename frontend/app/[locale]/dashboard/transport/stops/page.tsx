...
export default function TransportStopsPage() {
    ...
    const columns = useMemo<FilterColumn<Stop>[]>(() => [
        { key: "name", label: t("stops.stop"), accessor: stop => stop.name },
        { key: "route", label: t("stops.route"), accessor: stop => routeName(stop.route_id) },
        { key: "address", label: t("stops.address"), accessor: stop => stop.address },
    ...
    return (
        <div className="space-y-6">
            <div>
                <h1 className="text-2xl font-bold text-[#111827] dark:text-white">{t("stops.title")}</h1>
                <p className="mt-1 text-sm text-[#6B7280] dark:text-[#c7d0da]">{t("stops.subtitle")}</p>
            </div>

            <Card className="rounded-xl border border-[#E5E7EB] bg-white shadow-sm dark:border-[#3b4248] dark:bg-[#202528]">
                <CardHeader><CardTitle>{t("stops.addTitle")}</CardTitle></CardHeader>
                <CardContent className="grid gap-3 md:grid-cols-6">
                    ...
                    <input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} placeholder={t("stops.stopName")} className="rounded-md border px-3 py-2 text-sm dark:border-[#4a535b] dark:bg-transparent" />
                    <input type="number" value={form.sequence} onChange={e => setForm({ ...form, sequence: e.target.value })} placeholder={t("stops.order")} className="rounded-md border px-3 py-2 text-sm dark:border-[#4a535b] dark:bg-transparent" />
                    ...
                    <input value={form.scheduled_arrival} onChange={e => setForm({ ...form, scheduled_arrival: e.target.value })} placeholder={t("stops.etaPlaceholder")} className="rounded-md border px-3 py-2 text-sm dark:border-[#4a535b] dark:bg-transparent" />
                    <Button onClick={create} className="bg-black text-white hover:bg-black/90 md:col-span-6"><Plus className="mr-2 h-4 w-4" /> {t("stops.addButton")}</Button>
            ...
            <Card className="rounded-xl border border-[#E5E7EB] bg-white shadow-sm dark:border-[#3b4248] dark:bg-[#202528]">
                <CardHeader><CardTitle>{t("stops.list")} ({filter.filtered.length})</CardTitle></CardHeader>
                <CardContent className="overflow-x-auto">
                    <table className="w-full text-left text-sm">
                        <thead>
                            <tr className="border-b border-[#E5E7EB] text-[#6B7280] dark:border-[#3b4248]">
                                <th className="px-3 py-2">#</th><th className="px-3 py-2">{t("stops.stop")}</th><th className="px-3 py-2">{t("stops.route")}</th><th className="px-3 py-2">{t("stops.gps")}</th><th className="px-3 py-2">{t("stops.eta")}</th><th className="px-3 py-2 text-right">{t("common.actions")}</th>
                            </tr>
                        </thead>
                        <tbody>
                            {filter.filtered.length === 0 ? (
                                <tr><td colSpan={6} className="px-3 py-6 text-center text-[#6B7280]">{t("stops.empty")}</td></tr>
