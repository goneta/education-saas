...
export default function TransportRoutesPage() {
    ...
    const columns = useMemo<FilterColumn<Route>[]>(() => [
        { key: "name", label: "Trajet", accessor: route => route.name },
        { key: "driver", label: "Chauffeur", accessor: route => driverName(route.driver_id) },
        { key: "vehicle", label: "Véhicule", accessor: route => vehicleName(route.vehicle_id) },
    ...
    return (
        <div className="space-y-6">
            <div>
                <h1 className="text-2xl font-bold text-[#111827] dark:text-white">Trajets</h1>
                <p className="mt-1 text-sm text-[#6B7280] dark:text-[#c7d0da]">Définissez les trajets, leur chauffeur, véhicule et le tarif mensuel (intégré à la facturation).</p>
            </div>

            <Card className="rounded-xl border border-[#E5E7EB] bg-white shadow-sm dark:border-[#3b4248] dark:bg-[#202528]">
                <CardHeader><CardTitle>Ajouter un trajet</CardTitle></CardHeader>
                <CardContent className="grid gap-3 md:grid-cols-5">
                    <input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="Nom du trajet" className="rounded-md border px-3 py-2 text-sm dark:border-[#4a535b] dark:bg-transparent" />
                    <input type="number" value={form.monthly_fee} onChange={e => setForm({ ...form, monthly_fee: e.target.value })} placeholder="Tarif mensuel" className="rounded-md border px-3 py-2 text-sm dark:border-[#4a535b] dark:bg-transparent" />
            ...
            <Card className="rounded-xl border border-[#E5E7EB] bg-white shadow-sm dark:border-[#3b4248] dark:bg-[#202528]">
                <CardHeader><CardTitle>Trajets ({filter.filtered.length})</CardTitle></CardHeader>
                <CardContent className="overflow-x-auto">
                    <table className="w-full text-left text-sm">
                        <thead>
                            <tr className="border-b border-[#E5E7EB] text-[#6B7280] dark:border-[#3b4248]">
                                <th className="px-3 py-2">Trajet</th><th className="px-3 py-2">Chauffeur</th><th className="px-3 py-2">Véhicule</th><th className="px-3 py-2">Tarif / mois</th><th className="px-3 py-2 text-right">Actions</th>
                            </tr>
                        </thead>
                        <tbody>
                            {filter.filtered.length === 0 ? (
                                <tr><td colSpan={5} className="px-3 py-6 text-center text-[#6B7280]">Aucun trajet.</td></tr>
