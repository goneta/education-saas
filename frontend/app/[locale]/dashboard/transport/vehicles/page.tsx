...
export default function TransportVehiclesPage() {
    ...
    const columns = useMemo<FilterColumn<Vehicle>[]>(() => [
        { key: "name", label: t("vehicles.name"), accessor: vehicle => vehicle.name },
        { key: "type", label: t("vehicles.type"), accessor: vehicle => vehicle.vehicle_type },
        { key: "registration", label: t("vehicles.registration"), accessor: vehicle => vehicle.registration },
        { key: "status", label: t("vehicles.state"), accessor: vehicle => vehicle.status },
    ...
    return (
        <div className="space-y-6">
            <div>
                <h1 className="text-2xl font-bold text-[#111827] dark:text-white">{t("vehicles.title")}</h1>
                <p className="mt-1 text-sm text-[#6B7280] dark:text-[#c7d0da]">{t("vehicles.subtitle")}</p>
            </div>

            <Card className="rounded-xl border border-[#E5E7EB] bg-white shadow-sm dark:border-[#3b4248] dark:bg-[#202528]">
                <CardHeader><CardTitle>{t("vehicles.addTitle")}</CardTitle></CardHeader>
                <CardContent className="grid gap-3 md:grid-cols-5">
                    <input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} placeholder={t("vehicles.namePlaceholder")} className="rounded-md border px-3 py-2 text-sm dark:border-[#4a535b] dark:bg-transparent" />
                    ...
                    <input value={form.registration} onChange={e => setForm({ ...form, registration: e.target.value })} placeholder={t("vehicles.registration")} className="rounded-md border px-3 py-2 text-sm dark:border-[#4a535b] dark:bg-transparent" />
                    <input type="number" value={form.capacity} onChange={e => setForm({ ...form, capacity: e.target.value })} placeholder={t("vehicles.capacity")} className="rounded-md border px-3 py-2 text-sm dark:border-[#4a535b] dark:bg-transparent" />
            ...
            <Card className="rounded-xl border border-[#E5E7EB] bg-white shadow-sm dark:border-[#3b4248] dark:bg-[#202528]">
                <CardHeader><CardTitle>{t("vehicles.fleet")} ({filter.filtered.length})</CardTitle></CardHeader>
                <CardContent className="overflow-x-auto">
                    <table className="w-full text-left text-sm">
                        <thead>
                            <tr className="border-b border-[#E5E7EB] text-[#6B7280] dark:border-[#3b4248]">
                                <th className="px-3 py-2">{t("vehicles.name")}</th><th className="px-3 py-2">{t("vehicles.type")}</th><th className="px-3 py-2">{t("vehicles.registration")}</th><th className="px-3 py-2">{t("vehicles.capacity")}</th><th className="px-3 py-2">{t("vehicles.state")}</th><th className="px-3 py-2 text-right">{t("common.actions")}</th>
                            </tr>
                        </thead>
                        <tbody>
                            {filter.filtered.length === 0 ? (
                                <tr><td colSpan={6} className="px-3 py-6 text-center text-[#6B7280]">{t("vehicles.empty")}</td></tr>
