...
- Seconde passe (BUG-A): `rewrites()` s execute aussi au BUILD (`next build` force NODE_ENV=production); lever une exception sur BACKEND_INTERNAL_URL absente cassait la CI et tout pipeline de deploiement. Remplace par un avertissement au build, la variable restant verifiee au runtime par teducai-prod-audit.sh.
