...
- MODULE_BY_PATH maps /dashboard/assignments and /my-assignments -> assignments help section.
- Multi-select export fix: `exportRows(rows, format)` now builds ONE document from ALL rows (CSV/Excel: a single file with header + one line per row + UTF-8 BOM; PDF/Print: a single print window with a full table). Previously `bulkExport`/`bulkPrint` called the per-row exporter once per row, so N CSV/Excel downloads collided to one file and N print popups were blocked to one — only the first selected row appeared. `exportRow`/`printRow` are thin single-row wrappers; `bulkExport`=exportRows(bulkRows()), `bulkPrint`=printRows(bulkRows()). Reads rendered cells, so active filters, sort, visible columns and formatting are preserved.
- MODULE_BY_PATH: routes discipline/activities/health/boarding-school/education-exams -> section aide "school_life"; reference-data -> section "reference_data" (aide contextuelle).
