...
const Input = React.forwardRef<HTMLInputElement, InputProps>(
    ({ className, type, placeholder, title, "aria-label": ariaLabel, ...props }, ref) => {
        ...
        const inferredLabel = (typeof ariaLabel === "string" && ariaLabel) || props.name || props.id || type || (typeof localizedPlaceholder === "string" && localizedPlaceholder)
        const localizedAriaLabel = typeof ariaLabel === "string" ? localizeUiText(ariaLabel) : ariaLabel
        return (
            <input
                type={type}
                placeholder={localizedPlaceholder}
                title={title || helpForLabel(String(inferredLabel || "Ce champ"))}
                aria-label={localizedAriaLabel || (inferredLabel ? localizeUiText(String(inferredLabel)) : undefined)}
                className={cn(
