...
- S3/MinIO mode: object upload plus short-lived signed download URLs.
- Distributed rate limiting: `REDIS_URL` switches API abuse protection from local memory to Redis.
...
- Backup scripts: SQLite copy and PostgreSQL `pg_dump`/`pg_restore` wrappers with checksum metadata and optional encryption.
- Consent and retention registries for GDPR/GDPR-like operations.
- CSV/XLSX import endpoint for students.
...
- Configure `REDIS_URL` in multi-instance production.
